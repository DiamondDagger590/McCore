...
- Boundary values: is new logic tested with an empty collection, `0`, a negative value, and the maximum the type allows (`Integer.MAX_VALUE`, `Long.MAX_VALUE` for timestamps)? Mid-range "happy" inputs hide overflow and clamping bugs.
- Collaborator failure paths: when a dependency returns empty (a registry lookup misses, an absent `PluginHook` `Optional`, a DAO `ResultSet` with no rows) or throws (`SQLException` mid-transaction), is that branch exercised — not just the found/success branch?
- State machine transitions: for stateful types (`TransactionState`, `CoreTask` lifecycle, `ChatResponse` expiry), is there a test for an invalid transition — `runTask()` on an already-running task, executing an already-rolled-back transaction, responding to an already-expired `ChatResponse`? Testing only the legal path leaves illegal transitions with undefined behavior.
- Off-by-one: for any range, pagination, or index arithmetic (`PaginatedGui` page math, `LinkedNode` traversal), is the exact boundary tested — first index, last index, one-past-last, single-element page, exactly-full page?
- `Parser` mathematical edge cases: is there a test where a formula divides by zero, produces a negative result, overflows, or references an uninitialized variable? Downstream plugins feed `Parser` arbitrary config strings — the evaluator's failure behavior is API contract.
- Concurrent mutation during iteration: does tested code iterate a collection that a listener, callback, or task can mutate mid-iteration (e.g., `GuiManager` iterating tracked GUIs while a close handler untracks one)? A test that mutates during iteration catches `ConcurrentModificationException` before a downstream plugin does.
- Time boundaries: for expiration checks (`ExpireableCoreTask`, `ChatResponseExpireTask`, `StatisticCache` eviction), is the exact boundary instant tested (`now == expiresAt` via a fixed `TimeProvider`), not just clearly-before and clearly-after? Inclusive-vs-exclusive comparison bugs live exactly on the boundary.
- Config-driven edge cases: for `ReloadableContent` subclasses, is the load path tested when the YAML key is missing, the value is an empty string, or a non-numeric string sits where a number is expected — not only against well-formed values?
- Ordering assumptions: does any test assert on iteration order of a `HashSet`/`HashMap`-backed collection (e.g., registry contents)? Such tests pass by accident of hashing; assert order-insensitively or use an ordered structure in production.
...
- Does every test method carry a `@DisplayName` annotation with a descriptive label that clearly communicates the test's intent (e.g., `@DisplayName("Given a registered manager, when registering again, then throws IllegalArgumentException")`, `@DisplayName("throws when the manager is already registered")`)?
...
- Does any test interact with `RegistryAccess` singleton state without using `RegistryResetExtension` (via `@ExtendWith`) or `InternalResetTestTools` to reset it between tests? Leaked singleton state causes order-dependent test failures.

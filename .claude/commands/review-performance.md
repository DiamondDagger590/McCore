...
- Does any Bukkit event handler (especially `PlayerLoadEvent`, `PlayerUnloadEvent`, `GuiRefreshEvent`, or any event firing multiple times per second) contain a loop proportional to the number of players or registered objects? Flag O(n) or worse in these handlers.
...
- Is a new `Map` or `Set` field whose entries are inserted when players join or objects are registered but never removed when the player quits or the object is unregistered? Unbounded growth is a memory leak.
...
- Does any `StatisticCache` usage configure Caffeine without a max size or expiration? Unbounded Caffeine caches grow without limit.

**Resource Leaks**
- Does any code open a `Connection`, `PreparedStatement`, or `ResultSet` outside of a try-with-resources block? Any unclosed resource leaks a connection from the HikariCP pool permanently.
- Does any method construct an I/O stream (`InputStream`, `OutputStream`) without a try-with-resources or `finally` close?
- Is any `CoreTask` subclass scheduled without storing the returned `BukkitTask` handle so it can be cancelled on plugin disable?

**HikariCP Pool Usage**
- Does any code hold a `Connection` open across an async boundary (passing it to a `CompletableFuture` stage that runs on a different thread)? Connections should be acquired and released within a single execution block.
- Does any code acquire multiple `Connection`s from the pool in the same call stack without releasing the first? This can exhaust the pool under load.
- Does any synchronous database call happen on the main thread outside of `blockMainThreadOnStart()`? All runtime DB I/O must go through `Database.getDatabaseExecutorService()`.

**Scheduler Task Lifecycle**
- Does any new repeating task lack a guard that checks whether a previous instance is already running? `RepeatableCoreTask.runTask()` has a built-in guard — verify it is not bypassed.
...
- Does any constructor call `runTask()`, coupling object creation to the scheduler?

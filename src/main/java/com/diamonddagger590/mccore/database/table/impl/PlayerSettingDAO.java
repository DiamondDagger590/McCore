...
public class PlayerSettingDAO {
    ...
    private static final String TABLE_NAME = "player_settings";
    ...
    public static Set<PlayerSetting> getPlayerSettings(@NotNull Connection connection, @NotNull UUID playerUUID) {
        PlayerSettingRegistry playerSettingRegistry = CorePlugin.getInstance().registryAccess().registry(RegistryKey.PLAYER_SETTING);
     ...
     * @throws SettingNotRegisteredException If {@link PlayerSettingRegistry#isRegistered(NamespacedKey)}returns {@code false}.
     */
    @NotNull
